...
ColumnLayout {
  ...
  function tr(key, interpolations) {
    const vars = interpolations || {};
    if (pluginApi && pluginApi.tr)
      return pluginApi.tr(key, vars);
  ...
  function formatEdidSummary(summaryObj) {
    ...
    const lines = [];
    if (s.monitorName)
      lines.push(root.tr("panels.display.edid-field-monitor-name", { "value": s.monitorName }));
    if (s.manufacturerId)
      lines.push(root.tr("panels.display.edid-field-manufacturer-id", { "value": s.manufacturerId }));
    if (s.productCode)
      lines.push(root.tr("panels.display.edid-field-product-code", { "value": s.productCode }));

    const serialValue = s.serialText || s.serialNumber;
    if (serialValue)
      lines.push(root.tr("panels.display.edid-field-serial-number", { "value": serialValue }));

    if (s.version)
      lines.push(root.tr("panels.display.edid-field-version", { "value": s.version }));

    if (s.week !== null && s.year !== null) {
      lines.push(root.tr("panels.display.edid-field-manufactured", { "week": s.week, "year": s.year }));
    } else if (s.year !== null) {
      lines.push(root.tr("panels.display.edid-field-manufactured", { "week": root.tr("common.unknown"), "year": s.year }));
    }

    if (s.inputType)
      lines.push(root.tr("panels.display.edid-field-input-type", { "value": s.inputType }));
    ...
    if (widthCm !== null && heightCm !== null) {
      const sizeText = root.tr("panels.display.edid-size-cm", { "width": widthCm, "height": heightCm });
      lines.push(root.tr("panels.display.edid-field-physical-size", { "value": sizeText }));
    }

    if (s.preferredMode)
      lines.push(root.tr("panels.display.edid-field-preferred-mode", { "value": s.preferredMode }));

    if (s.source)
      lines.push(root.tr("panels.display.edid-field-decoder", { "value": s.source }));
  ...
  function formatEdidDetails(rawHex, decodedText, showRawDecoded) {
    ...
    if (showRawDecoded === true) {
      ...
      return root.tr("panels.display.edid-empty");
    }
  ...
  function buildEdidCopyPayload() {
    ...
    const sections = [
      root.tr("panels.display.edid-title", { "output": output }),
      "",
      shown
    ];

    if (decodeError !== "")
      sections.push("", root.tr("panels.display.edid-section-decode-error"), decodeError);

    if (decoded.trim() !== "" && decoded.trim() !== shown.trim())
      sections.push("", root.tr("panels.display.edid-section-decoded-output"), decoded);
  ...
  NBox {
    ...
    RowLayout {
      ...
      ColumnLayout {
        ...
        spacing: 2
        NText {
          text: root.tr("panels.display.hw-control-unavailable") || "Hardware Control Unavailable"
        ...
        NText {
          text: root.tr("panels.display.hw-control-unavailable-desc") || "Rendering in read-only mode."
  ...
  ColumnLayout {
    ...
    spacing: Style.marginL
      ...
      NText {
        text: root.tr("panels.display.monitor-layout")
      ...
      NText {
        text: root.tr("panels.display.monitor-drag-info")
        pointSize: Style.fontSizeS
        color: Color.mOnSurfaceVariant
      }
      ...
      NText {
        text: root.tr("panels.display.monitor-settings")
        pointSize: Style.fontSizeXL
        font.weight: Style.fontWeightBold
        color: Color.mOnSurface
      }
      ...
      Repeater {
        ...
        delegate: NBox {
          ...
          readonly property string automaticOptionLabel: {
            var baseLabel = root.tr("panels.display.monitors-backlight-device-auto-option");
          ...
          ColumnLayout {
            ...
            RowLayout {
              ...
              ColumnLayout {
                ...
                NText {
                  text: {
                    ...
                    return parts.join(" · ") || root.tr("common.unknown");
                  }
                  pointSize: Style.fontSizeXS
                  color: Color.mOnSurfaceVariant
                  elide: Text.ElideRight
              ...
              NIconButton {
                icon: "info-circle"
                tooltipText: root.tr("panels.display.read-edid")
                onClicked: {
            ...
            GridLayout {
              ...
              ColumnLayout {
                ...
                NText { text: root.tr("common.position"); color: Color.mOnSurfaceVariant; pointSize: Style.fontSizeXS }
                NText { text: "(" + (logicalData.x || 0) + ", " + (logicalData.y || 0) + ")"; color: Color.mOnSurface; pointSize: Style.fontSizeS }
              }
              ColumnLayout {
                spacing: 2
                NText { text: root.tr("panels.display.logical-size"); color: Color.mOnSurfaceVariant; pointSize: Style.fontSizeXS }
                NText { text: (logicalData.width || 0) + "×" + (logicalData.height || 0); color: Color.mOnSurface; pointSize: Style.fontSizeS }
              }
              ColumnLayout {
                spacing: 2
                NText { text: root.tr("common.scale"); color: Color.mOnSurfaceVariant; pointSize: Style.fontSizeXS }
                NText { text: (logicalData.scale || 1.0).toFixed(2) + "x"; color: Color.mOnSurface; pointSize: Style.fontSizeS }
              }
              ColumnLayout {
                spacing: 2
                NText { text: root.tr("panels.display.rotation"); color: Color.mOnSurfaceVariant; pointSize: Style.fontSizeXS }
                NText { 
                  text: {
                    ...
                    if (t === "180") return "180°";
                    if (t === "270") return "270°";
                    if (t.startsWith("Flipped")) return root.tr("panels.display.flipped") || "Flipped";
                    return root.tr("common.normal");
                  }
                  color: Color.mOnSurface; 
                  pointSize: Style.fontSizeS 
                }
            ...
            RowLayout {
              ...
              NText {
                text: root.tr("common.brightness")
            ...
            NText {
              ...
              text: !Settings.data.brightness.enableDdcSupport ? root.tr("panels.display.monitors-brightness-unavailable-ddc-disabled") : root.tr("panels.display.monitors-brightness-unavailable-generic")
              pointSize: Style.fontSizeXS
              color: Color.mOnSurfaceVariant
              Layout.fillWidth: true
              wrapMode: Text.WordWrap
            ...
            NComboBox {
              ...
              label: root.tr("panels.display.monitors-backlight-device-label")
              description: root.tr("panels.display.monitors-backlight-device-description")
            ...
            NComboBox {
              ...
              enabled: monitorCard.isEnabled
              Layout.fillWidth: true
              label: root.tr("common.resolution")
            ...
            NComboBox {
              ...
              enabled: monitorCard.isEnabled
              Layout.fillWidth: true
              label: root.tr("panels.display.refresh-rate")
            ...
            NComboBox {
              ...
              enabled: monitorCard.isEnabled
              Layout.fillWidth: true
              label: root.tr("panels.display.rotation")
              model: [
                { key: "Normal", name: root.tr("common.normal")},
                { key: "90", name: "90°" },
                { key: "180", name: "180°" },
                { key: "270", name: "270°" },
                { key: "Flipped", name: root.tr("panels.display.flipped") },
                { key: "Flipped90", name: root.tr("panels.display.flipped-angle", { "angle": "90°" }) },
                { key: "Flipped180", name: root.tr("panels.display.flipped-angle", { "angle": "180°" }) },
                { key: "Flipped270", name: root.tr("panels.display.flipped-angle", { "angle": "270°" }) }
              ]
            ...
            NToggle {
              ...
              enabled: monitorCard.isEnabled
              Layout.fillWidth: true
              label: root.tr("panels.display.vrr")
            ...
            NToggle {
              ...
              Layout.fillWidth: true
              label: root.tr("common.enable-display")
              checked: {
      ...
      NSpinBox {
        Layout.fillWidth: true
        label: root.tr("panels.display.monitors-brightness-step-label")
        description: root.tr("panels.display.monitors-brightness-step-description")
        minimum: 1
        maximum: 50
      ...
      NToggle {
        Layout.fillWidth: true
        label: root.tr("panels.display.monitors-enforce-minimum-label")
        description: root.tr("panels.display.monitors-enforce-minimum-description")
      ...
      NToggle {
        Layout.fillWidth: true
        label: root.tr("panels.display.monitors-external-brightness-label")
        description: root.tr("panels.display.monitors-external-brightness-description")
  ...
  Popup {
    ...
    contentItem: ColumnLayout {
      ...
      RowLayout {
        ...
        ColumnLayout {
          ...
          NText {
            text: root.tr("panels.display.edid-title", {
          ...
          NText {
            text: root.tr("panels.display.edid-description")
            pointSize: Style.fontSizeM
            color: Color.mOnSurfaceVariant
            wrapMode: Text.WordWrap
            Layout.fillWidth: true
        ...
        NButton {
          text: root.tr("panels.display.edid-view-original")
      ...
      RowLayout {
        ...
        NButton {
          text: root.tr("panels.display.copy-edid")
    ...
    Component {
      ...
      Item {
        NText {
          anchors.fill: parent
          text: displayService.edidError || root.tr("panels.display.edid-empty")
          pointSize: Style.fontSizeS
          color: Color.mError
          wrapMode: Text.WordWrap
    ...
    Process {
      ...
      stderr: StdioCollector {
        onStreamFinished: {
          const err = text.trim();
          if (err.length > 0)
            ToastService.showWarning(root.tr("panels.display.read-edid"), err);
        }
      }

      onExited: function(exitCode) {
        if (exitCode === 0) {
          ToastService.showNotice(root.tr("panels.display.read-edid"), root.tr("common.copied-to-clipboard"));
        } else {
          ToastService.showWarning(root.tr("panels.display.read-edid"), root.tr("panels.display.edid-copy-failed"));
        }
      }
    }
  }
  ...
  Popup {
    ...
    contentItem: ColumnLayout {
      ...
      NText {
        text: root.tr("panels.display.keep-changes")
        pointSize: Style.fontSizeXL
        font.weight: Style.fontWeightBold
        color: Color.mOnSurface
        Layout.alignment: Qt.AlignHCenter
      ...
      NText {
        text: root.tr("panels.display.reverting-in-seconds", {
